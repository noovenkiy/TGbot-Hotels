...
def city_options(message: Message, city: str) -> None:
    ...
    if cities is None:
        ...
        with bot.retrieve_data(user_id, chat_id) as data:
            data["city"] = cities
        bot.send_message(
            chat_id, "Уточните поиск", reply_markup=city_choice_keyboard(cities)
        )
    else:
        bot.send_message(
            chat_id,
            f'По запросу "{city}" ничего не найдено.\n'
...
def city_received(message: Message) -> None:
    ...
    if message.text in cities:
        ...
        bot.send_message(
            message.chat.id, "Уточните поиск", reply_markup=city_choice_keyboard(cities)
        )
