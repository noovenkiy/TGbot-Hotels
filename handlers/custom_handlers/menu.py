...
def main_menu_st2(call: CallbackQuery) -> None:
    ...
    if isinstance(call, CallbackQuery):
        bot.edit_message_reply_markup(chat_id, call.message.id, reply_markup=main_menu_st2_keyboard(number_hotels, foto))
    else:

        bot.send_message(chat_id, f"Выбранный город:\t{city}\n"
                                  f"Дата заезда:\t{check_in}\n"
                                  f"Дата отъезда:\t{check_out}\n"
                                  f"Всего дней:\t{amount_days}", reply_markup=main_menu_st2_keyboard(number_hotels, foto))
